-- ================================================================
-- Workload Mix: READ-HEAVY (95% reads, 5% updates) - Q1
-- ================================================================
-- 100 operations: 95 queries, 5 updates
-- Updates at operations: 20, 40, 60, 80, 100
-- ================================================================

\timing on

\echo ================================================================
\echo READ-HEAVY WORKLOAD (95/5) - Q1
\echo ================================================================

-- Clear previous results
DELETE FROM workload_results WHERE experiment_id = 'read_heavy_q1';
...
-- ================================================================
\echo === PRE-EXPERIMENT BASELINE ===

-- Q1(G)
INSERT INTO workload_results 
SELECT 'read_heavy_q1', 0, 'baseline', 'G', 
...
-- Q1(MV)
INSERT INTO workload_results 
SELECT 'read_heavy_q1', 0, 'baseline', 'MV', 
       EXTRACT(EPOCH FROM clock_timestamp()) * 1000, 
...
-- Q1(SSR)
INSERT INTO workload_results 
SELECT 'read_heavy_q1', 0, 'baseline', 'SSR', 
       EXTRACT(EPOCH FROM clock_timestamp()) * 1000, 
...
-- ================================================================
-- WORKLOAD EXECUTION (100 operations)
-- Read operations: 1-19, 21-39, 41-59, 61-79, 81-99
-- Update operations: 20, 40, 60, 80, 100
-- ================================================================

-- Helper: Run Q1 queries and record times
-- We'll use DO blocks to simulate the workload
...
DECLARE
    ...
    end_ts TIMESTAMP;
    q_result INT;
    update_ops INT[] := ARRAY[20, 40, 60, 80, 100];
BEGIN
    FOR op_num IN 1..100 LOOP
        IF op_num = ANY(update_ops) THEN
            -- UPDATE OPERATION
            RAISE NOTICE 'Operation %: UPDATE', op_num;
            
            -- Insert edges from batch
            INSERT INTO e_g (_0, _1, _2, _3)
            SELECT _0, _1, _2, _3 FROM update_batch WHERE batch_id = (op_num / 20) - 1;
            
            -- Record update time
            INSERT INTO workload_results VALUES 
                ('read_heavy_q1', op_num, 'update', 'BASE', 0, 10);
        ...
        ELSE
            ...
            INSERT INTO workload_results VALUES 
                ('read_heavy_q1', op_num, 'query', 'SSR', 
                 EXTRACT(EPOCH FROM (end_ts - start_ts)) * 1000, q_result);
        END IF;
...
-- Q1(G)
INSERT INTO workload_results 
SELECT 'read_heavy_q1', 999, 'post', 'G', 
       EXTRACT(EPOCH FROM clock_timestamp()) * 1000, 
       COUNT(DISTINCT n_person1._0)
FROM n_g n_person1
JOIN e_g e1 ON e1._1 = n_person1._0 AND e1._3 = 'KNOWS'
...
-- Q1(MV)
INSERT INTO workload_results 
SELECT 'read_heavy_q1', 999, 'post', 'MV', 
       EXTRACT(EPOCH FROM clock_timestamp()) * 1000, 
       COUNT(DISTINCT n_person1.node_id)
FROM v1_nodes n_person1
JOIN v1_edges e1 ON e1.from_node = n_person1.node_id AND e1.label = 'KNOWS'
...
-- Q1(SSR)
INSERT INTO workload_results 
SELECT 'read_heavy_q1', 999, 'post', 'SSR', 
       EXTRACT(EPOCH FROM clock_timestamp()) * 1000, 
       COUNT(DISTINCT person1)
FROM INDEX_v1 WHERE person1 >= 723388 AND person1 <= 723415;
...
-- ================================================================
\echo ================================================================
\echo READ-HEAVY Q1 RESULTS SUMMARY
\echo ================================================================

\echo Query times by mode (ms):
SELECT query_mode, 
...
WHERE experiment_id = 'read_heavy_q1' 
...
FROM workload_results 
WHERE experiment_id = 'read_heavy_q1' AND operation_type = 'update';

-- Cleanup: remove inserted edges
DELETE FROM e_g WHERE _0 >= 7000000;
\echo Cleanup: removed test edges from E_g
